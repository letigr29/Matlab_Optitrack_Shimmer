...
%%
med_imu_s=sincronizar_imus(med_cam,{med_imu},4769-929);
plot_camara_imu_2D(med_cam,med_imu_s);
%%

[m_ali0,m_ali]=matriz_ali(med_cal,med_imu_s,100)
...
for t=1:length(med_imu_s{1}.Quat)
    med_imu_s{1}.euler2quat(t,:)=(dcm2quat((m_ali)*quat2dcm(med_imu_s{1}.Quat(t,:))*m_ali0'));
end
...
%     med_cam.Rigid_Body.RigidBody.euler2quat(t,:)=dcm2quat(quat2dcm(med_cam.Rigid_Body.RigidBody.Rotation(t,:)));
% end
%%
plot_camara_imu_2D(med_cam,med_imu_s);
